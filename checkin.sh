echo '------------------sign------------------'
curl -H "cookie:${COOKIE}" -H "Authorization:6508525923683601371858261555053-1080-1920" -H 'content-type:application/json;charset=UTF-8' -d '{"token": "glados.one"}' -X POST 'https://glados.network/api/user/checkin' | grep -Eo '"message":"[^"]*"'
echo '-----------------status-----------------'
