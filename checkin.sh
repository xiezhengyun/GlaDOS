echo '------------------sign------------------'
curl -H "cookie:${COOKIE}" -H "24657105248964086372031919083781-1080-1920" -H 'content-type:application/json;charset=UTF-8' -d '{"token": "glados.one"}' -X POST 'https://glados.network/api/user/checkin' | grep -Eo '"message":"[^"]*"'
echo '-----------------status-----------------'
